...
.setThumbnail()
.addField(`HamHell | Teşekkürler`, `**Selamlar, Ben HamHell (HamHell'nin Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`HamHell | Prefix`, `**HamHell Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`HamHell | Nasıl Kullanılır?`, `**HamHell botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`HamHell | Linkler`, `**Sohbet Kanalına H!davet Yazmanız Yeterlidir**`)
.setFooter(`HamHell | Gelişmiş Türkçe Bot | 2021`)
.setTimestamp();
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setFooter(`HamHell Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
